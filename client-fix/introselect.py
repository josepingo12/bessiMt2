		...
		if -1 != self.startIndex:
			...
			if app.GetTime() - self.startReservingTime > 0.1:
				if False == self.openLoadingFlag:
					...
					chrSlot=self.stream.GetCharacterSlot()
					try:
						addr=net.GetAccountCharacterSlotDataInteger(chrSlot, net.ACCOUNT_CHARACTER_SLOT_ADDR)
						port=net.GetAccountCharacterSlotDataInteger(chrSlot, net.ACCOUNT_CHARACTER_SLOT_PORT)
						dbg.TraceError("[DBG] SLOT addr=" + str(addr) + " port=" + str(port))
					except: pass
					dbg.TraceError("[DBG] Calling SendSelectCharacterPacket(" + str(chrSlot) + ")")
					net.SendSelectCharacterPacket(chrSlot)
					dbg.TraceError("[DBG] SendSelectCharacterPacket returned, manually transitioning to LoadingPhase")
					self.stream.SetLoadingPhase()
					dbg.TraceError("[DBG] SetLoadingPhase manual call done")
					self.openLoadingFlag = True
					dbg.TraceError("[DBG] flag set, getting playTime, slot=" + str(self.slot))

					playTime=net.GetAccountCharacterSlotDataInteger(self.slot, net.ACCOUNT_CHARACTER_SLOT_PLAYTIME)
					dbg.TraceError("[DBG] got playTime=" + str(playTime) + " importing player")

					import player
					dbg.TraceError("[DBG] player imported, calling SetPlayTime")
					player.SetPlayTime(playTime)
					dbg.TraceError("[DBG] SetPlayTime done, importing chat")
					import chat
					dbg.TraceError("[DBG] chat imported, calling chat.Clear()")
					chat.Clear() ## ���� Chat �� �ʱ�ȭ. �ӽ� Pos.
					dbg.TraceError("[DBG] chat.Clear() done, end of DirectEnter sequence")
			## Temporary
		#######################################################
